...
{
  generer_terrain(T, l, h, dObst, graine);
...
void generer_terrain(Terrain *T, int l, int h, float dObst, int graine)
{
  ...
  for (i = 0; i < h; i++)
  {
    ...
    for (j = 0; j < l; j++)
    {
      ...
      if ((int)h / 2 == i && (int)l / 2 == j)
      {
        T->tab[i][j] = LIBRE;
      }
      else
      {
        T->tab[i][j] = rand_case(dObst, graine);
      }
    }
  }
}
...
float densite_obstacles(Terrain T)
{
  ...
  return nb_obstacles / (T.hauteur * T.largeur);
}
